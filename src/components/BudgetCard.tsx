...
export const BudgetCard = ({ budget, onClick }: BudgetCardProps) => {
  return (
    <button
      onClick={onClick}
      className="w-full card-elevated p-4 text-left transition-all duration-200 hover:-translate-y-0.5 hover:border-primary/35 hover:shadow-[var(--shadow-glow)]"
    >
      <div className="flex items-start justify-between gap-3">
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 mb-1">
            <span className={cn('text-xs px-2 py-0.5 rounded-full border border-white/10', CATEGORY_COLORS[budget.category])}>
              {CATEGORY_LABELS[budget.category].split(' ')[0]}
            </span>
            <span className={cn('text-xs px-2 py-0.5 rounded-full border border-white/10', statusStyles[budget.status])}>
